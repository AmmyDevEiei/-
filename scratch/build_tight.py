...
def build_tight():
    ...
    # =========================================================================
    # 4. ขั้นตอนการผลิตสื่อ (Starts on Page 3)
    # =========================================================================
    add_para(doc, "4. ขั้นตอนการผลิตสื่อ", font_size=18, bold=True, space_before=10, space_after=3, keep_with_next=True)
    ...
    add_para(doc, 
        ...
        font_size=16, space_before=0, space_after=4)

    # 4.1 Production Workflow
    add_para(doc, "4.1 ภาพรวมขั้นตอนและกระบวนการผลิตวิดีโอ (Production Workflow)", font_size=16, bold=True, space_before=5, space_after=2, keep_with_next=True)
    
    add_para(doc, "การสร้างวิดีโอการสอนความยาวประมาณ 3-4 นาทีด้วย AI ให้ได้คุณภาพสูง ชัดเจน และตรงตามหลักสูตร จำเป็นต้องแยกทำทีละฉาก (Scene-by-Scene) และใช้คำสั่งที่จำเพาะเจาะจง โดยมี 4 ขั้นตอนหลักในการดำเนินงาน:", font_size=16, space_after=3)
    ...
    # Step 3
    add_para(doc, "3) การซ้อนข้อความ กราฟิก และการแสดงลิขสิทธิ์ (Text & Graphics Overlay):", font_size=16, bold=True, space_before=3, space_after=1, keep_with_next=True)
    add_para(doc, 
    ...
    add_para(doc, 
        ...
        font_size=15, space_after=5)

    # 4.2 AI Tools Table
    add_para(doc, "4.2 การประยุกต์ใช้เครื่องมือเทคโนโลยีและปัญญาประดิษฐ์ (AI Tools & Applications)", font_size=16, bold=True, space_before=5, space_after=3, keep_with_next=True)
    ...
    # 4.3 Storyboard Table -> Start on clean page for full view
    add_para(doc, "4.3 ตารางสรุปภาพรวมโครงสร้างสื่อวิดีโอ (Video Storyboard Overview)", font_size=16, bold=True, space_before=6, space_after=3, keep_with_next=True, page_break_before=True)
    ...
    # 4.4 Scene by scene details -> Start on clean page
    ...
    def add_scene_section(scene_no, title, duration, video_prompt, voiceover, graphics, sfx, sample_img=None, img_caption="", page_break=False):
        p_hdr = add_para(doc, f"ฉากที่ {scene_no}: {title} (ช่วงเวลา {duration})", font_size=16, bold=True, color=(15, 23, 42), space_before=5, space_after=2, keep_with_next=True, page_break_before=page_break)
        ...
        p_vo = add_para(doc, space_before=2, space_after=2, keep_with_next=True)
        ...
        if sample_img and os.path.exists(sample_img):
            ...
            r_img = p_img.add_run()
            r_img.add_picture(sample_img, width=Inches(4.4))
            ...
            add_para(doc, "", font_size=3, space_before=0, space_after=2)
    ...
    # 4.5 Copyright & 5. Link & QR Code -> Final Page (Page Break)
    add_para(doc, "4.5 การวางสัญลักษณ์แสดงลิขสิทธิ์ประจำตัวนิสิต (Copyright Identification)", font_size=16, bold=True, space_before=6, space_after=2, keep_with_next=True, page_break_before=True)
    ...
    add_para(doc, "5. ลิงก์สื่อ Video และ QR Code สำหรับเข้าถึง", font_size=18, bold=True, space_before=8, space_after=3, keep_with_next=True)
    ...
    p_link = add_para(doc, "ลิงก์ Google Drive: ", font_size=16, bold=True, space_after=4)
    ...
    if os.path.exists(qr_path):
        ...
        run_qr = p_qr.add_run()
        run_qr.add_picture(qr_path, width=Inches(2.0))
    ...
    print(f"Tight document successfully written to: {target_docx}")
