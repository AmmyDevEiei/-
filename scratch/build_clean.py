...
def build_clean():
    ...
    # =========================================================================
    add_para(doc, "4. ขั้นตอนการผลิตสื่อ", font_size=18, bold=True, space_before=10, space_after=4, keep_with_next=True)
    ...
    add_para(doc, "", font_size=6, space_before=0, space_after=3)

    # -------------------------------------------------------------------------
    # 4.3 ตารางสรุปภาพรวมโครงสร้างสื่อวิดีโอ (Video Storyboard Overview)
    # -------------------------------------------------------------------------
    add_para(doc, "4.3 ตารางสรุปภาพรวมโครงสร้างสื่อวิดีโอ (Video Storyboard Overview)", font_size=16, bold=True, space_before=6, space_after=3, keep_with_next=True)
    ...
    # -------------------------------------------------------------------------
    # 4.4 รายละเอียดชุดคำสั่ง AI และการผลิตสื่อแยกรายฉาก
    # -------------------------------------------------------------------------
    add_para(doc, "4.4 รายละเอียดชุดคำสั่ง AI และการผลิตสื่อแยกรายฉาก (Scene-by-Scene Production & AI Video Prompts)", font_size=16, bold=True, space_before=8, space_after=3, keep_with_next=True)
    ...
    def add_scene_section(scene_no, title, duration, video_prompt, voiceover, graphics, sfx, sample_img=None, img_caption=""):
        ...
        p_sfx = add_para(doc, space_before=2, space_after=4, keep_with_next=(sample_img is not None))
    ...
    # -------------------------------------------------------------------------
    add_para(doc, "4.5 การวางสัญลักษณ์แสดงลิขสิทธิ์ประจำตัวนิสิต (Copyright Identification)", font_size=16, bold=True, space_before=8, space_after=3, keep_with_next=True)
    ...
    if os.path.exists(qr_path):
        ...
        p_qr.paragraph_format.space_before = Pt(12)
